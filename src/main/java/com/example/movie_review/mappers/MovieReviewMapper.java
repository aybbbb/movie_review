...
public interface MovieReviewMapper {
    ...
    @Select(" SELECT r.id, r.movie_id, r.mem_id, r.text_review, r.photo_review, r.rating, r.is_del, r.reg_date, r.edit_date, "+
            ...
            " Where r.movie_id = #{movieId} and r.is_del  = 0 "
    )
    @Results(id= "movieReviewList")
    public List<Review> moveiReiviewList(String movieId);
    ...
    @Select(" SELECT IFNULL(r.id, 0) AS id" +
            " From reviews as r "+
            " Where r.movie_id = #{movieId} and r.mem_id = #{memId} and r.is_del  = 0 "+
            " LIMIT 1 "
    )  
    public Integer myReviewId(@Param("movieId")  String movieId, @Param("memId") int memId);



    @Select("SELECT IFNULL(ROUND(AVG(rating),1),0) AS ratingAvg FROM reviews WHERE movie_id =#{movieId} and is_del = 0")
    public float selectRatingAvg(String movieId);

    @Insert("Insert into reviews (mem_id, movie_id, text_review, photo_review, rating, is_del, reg_date, edit_date ) "+
            "Values ( #{memId}, #{movieId}, #{textReview}, #{photoReview}, #{rating}, 0 , NOW(), NOW() ) "
    )
